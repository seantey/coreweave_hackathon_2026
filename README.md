...
**Current target gap:** the user requested an immersive Marble Gaussian-splat office, and the Hunyuan result does not satisfy that visual target. The viewer now supports full-window exploration and a collision-checked virtual probe, tested with a clearly labeled prior Marble bedroom (`/?scene=stream-fixture&immersive=1`). That bedroom is an integration fixture, not the restored office. Two further office generation attempts failed in Mint's upstream preview provider with HTTP 403 / exhausted balance. Mint OAuth itself works; no office Marble world exists yet.
