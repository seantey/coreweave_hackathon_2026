...
Project isolation correction: the assistant improperly imported an unrelated prior-project scene, its collider and camera calibration for testing, then linked it as viewer progress. The user explicitly rejected this. That scene, derived screenshots/captures, provider context dump containing unrelated assets, and dependent browser test have been removed from this workspace. Do not restore them. Historical claims about that test are not validation of the office.

Expand scene hides the workspace panels. Pilot virtual probe enables drag-to-look, WASD movement and Q/E vertical movement. Escape returns to orbit. Reset view restores the first saved camera; Show collision mesh toggles the actual imported collider. The probe is a kinematic sphere of radius 0.12 **scene units**, not a human capsule, gravity simulation, or calibrated robot. Two-sided triangle distance checks and substeps stop it at mesh contact. Gaps in the generated collider remain traversable; lack of contact does not prove real-world free space. Do not infer that all visible walls are closed colliders.
