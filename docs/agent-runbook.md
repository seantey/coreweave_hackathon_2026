...
## Historical checkpoint — September 13, 10:49 AM PDT
...
## Immersive viewer and Marble blocker — September 13, after 11:30 AM PDT

The requested deliverable remains an immersive Marble splat office. Hunyuan completion does not fulfill that target. Mint OAuth and reference upload work, but both new requests failed in `fal-ai/gemini-3.1-flash-image-preview` with HTTP 403: `User is locked. Reason: Exhausted balance.` Omitting the prompt did not skip that provider. Reauth is not a fix for this error; avoid duplicate submissions until service access changes. Records are in `data/world-generation/marble-clean-office/` and `marble-image-only/`. The former includes the accepted input's JPEG upload derivative, preserving the original PNG. Initial PNG upload exceeded the gateway body limit (413); JPEG upload succeeded. There is no pending generation job at this checkpoint.

When Mint tools were absent from the session catalog, a local Codex app-server connection reused stored OAuth and issued direct MCP calls. It did not invoke a model turn or extract credentials. Local diagnostic scripts and schemas remain under `.artifacts/`; this is an operator recovery route, not a deployed application integration. The optional direct World Labs adapter still requires a separate key, which is absent from the configured file.

Open `http://127.0.0.1:8000/?scene=stream-fixture&immersive=1` for the existing Marble bedroom integration fixture. The local scene's shared splat/collider transform and Entry/Bedside/Window cameras now use the calibration of that exact prior-project asset. Those values must not be copied to a new office. No authored house cutouts, stairs or floor meshes were imported. The fixture description and goal explicitly identify its testing purpose.

Expand scene hides the workspace panels. Pilot virtual probe enables drag-to-look, WASD movement and Q/E vertical movement. Escape returns to orbit. Reset view restores the first saved camera; Show collision mesh toggles the actual imported collider. The probe is a kinematic sphere of radius 0.12 **scene units**, not a human capsule, gravity simulation, or calibrated robot. Two-sided triangle distance checks and substeps stop it at mesh contact. Gaps in the generated collider remain traversable; lack of contact does not prove real-world free space. A forward test path exited without contact; a downward path stopped at the actual generated floor. Do not infer that all visible walls are closed colliders.
...
Validation: `node scripts/probe-navigation-check.mjs` (Node 22.18+ for direct TypeScript loading) covers thin walls, back faces, clear paths, changed geometry transforms and missing colliders. `node scripts/immersive-check.mjs` requires the local calibrated `stream-fixture` and both servers; it checks keyboard camera translation, an actual Marble floor contact, overlays and Escape, with no page errors. Evidence is `.artifacts/immersive-check.json` and `marble-immersive-*.png`. The fixture remains private local data. Tests establish tool mechanics, not autonomous scene repair or real-world correctness.
