...
def workspace(tmp_path, monkeypatch):
    ...
    scene = Scene(id='fixture', title='Test fixture', description='Synthetic unit test',
        source_kind='synthetic_fixture', assets=[Asset(id='room', label='Room', kind='splat')],
        cameras={name: Camera() for name in ('Front', 'Side', 'Back')},
        bounds=Bounds(minimum=(-5,-1,-5), maximum=(5,4,5)),
        revisions=[Revision(id='original', label='Original', status='baseline', created_at=storage.timestamp())],
        current_revision='original')
    storage.save_scene(scene)
    captures=[]
    def capture(scene_id, revision, camera):
        name=f'{revision}-{camera}.png'
        Image.new('RGB', (32,32), 'black' if revision=='original' else 'white').save(tmp_path/name)
        captures.append((revision,camera))
        return {'image':name,'scene_id':scene_id,'revision_id':revision,'camera_name':camera,
                'camera_matrix':[1,0,0,1], 'projection_matrix':[1,0,0,1], 'viewport':[32,32]}
    monkeypatch.setattr(agent,'capture',capture)
    monkeypatch.setattr(agent,'observe_scene',lambda *args: {'assessment':{
        'next_action':'repair','edit':{'operation':'hide_region','asset_id':'room',
        'reason':'Known synthetic test defect','evidence':['Front'],
        'bounds':{'minimum':[0,0,0],'maximum':[.5,1,.5]}}}})
    return tmp_path, captures


def model_result(**overrides):
    value={'defect_resolved':True,'furniture_preserved':True,'new_visible_damage':False,
           'evidence':'The fixture change is visible in all supplied views.', 'uncertainties':[]}
    value.update(overrides)
    return {'text':json.dumps(value)}


@pytest.mark.parametrize('overrides,expected', [({},'accepted'),
...
def test_loop_decisions_and_held_out_camera(workspace,monkeypatch,overrides,expected):
    monkeypatch.setattr(agent.providers,'vision',lambda *args:model_result(**overrides))
...
def test_malformed_evaluator_cannot_accept_candidate(workspace,monkeypatch):
    monkeypatch.setattr(agent.providers,'vision',lambda *args:model_result(defect_resolved='true'))
    with pytest.raises(ValueError):agent.repair_loop('fixture',1)
    scene=storage.read_scene('fixture')
...
def test_identical_views_reject_without_paid_judgment(workspace,monkeypatch):
    ...
    def forbidden(*args):raise AssertionError('Unchanged candidate should not invoke inference')
...
def test_camera_mismatch_rejects_candidate(workspace,monkeypatch):
    ...
    def moved(scene_id,revision,camera):
        ...
        if revision!='original':record['camera_matrix']=[2,0,0,1]
        return record
    monkeypatch.setattr(agent,'capture',moved)
    with pytest.raises(ValueError,match='camera mismatch'):agent.repair_loop('fixture',1)
