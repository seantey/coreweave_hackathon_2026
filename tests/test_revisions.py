...
def scene(tmp_path,monkeypatch):
    ...
    s=Scene(id='office',title='Office',description='Test',assets=[Asset(id='room',label='Room',kind='splat')],
        cameras={'Front':Camera()},bounds=Bounds(minimum=(-5,-1,-5),maximum=(5,4,5)),
        revisions=[Revision(id='original',label='Original',status='baseline',created_at=storage.timestamp())],current_revision='original')
    storage.save_scene(s)
    return s


def removal(id='remove-person'):
    return Edit(id=id,operation='hide_region',asset_id='room',reason='Remove identified person',evidence=['Front','Side'],bounds=Bounds(minimum=(0,0,0),maximum=(.5,1.8,.5)))
...
def test_mass_erasure_is_rejected(scene):
    ...
    with pytest.raises(ValueError,match='12%'):storage.propose(scene.id,edit)


def test_edits_accumulate_without_mutating_original(scene):
    first=storage.propose(scene.id,removal('first'));storage.decide(scene.id,first.id,True,{})
...
def test_asset_path_cannot_escape_data(scene):
    with pytest.raises(ValueError):storage.media_path('../.env')


def test_invalid_geometry_is_rejected():
    with pytest.raises(ValidationError):Bounds(minimum=(0,0,0),maximum=(0,1,1))
