    ...
    with path.open("rb") as f:
        ...
        while True:
            ...
            if line.startswith("property "):
                ...
                if kind != "float":
                    raise ValueError("Expected Gaussian float properties")
...
def mesh_points(path: Path, maximum=2500):
    with path.open("rb") as f:
        ...
        if magic != b"glTF" or version != 2:
            raise ValueError("Expected GLB version 2")
...
def fit(splat: Path, mesh: Path, metadata: dict):
    target = splat_centers(splat)
    source = mesh_points(mesh)
    ...
    def residual(points):
        return float(
            (
                target_tree.query(points)[0].mean()
                + cKDTree(points).query(target)[0].mean()
            )
            / 2
        )
    ...
    for candidate in candidates[:8]:
        ...
        for _ in range(60):
            ...
            keep = distances <= np.quantile(distances, 0.95)
            a = points[keep]
            ...
            matrix = delta @ matrix
            if (
                np.linalg.norm(offset) < 1e-7
                and np.linalg.norm(rotation - np.eye(3)) < 1e-6
            ):
                break
    ...
    return {
        "method": "metadata convention search followed by trimmed rigid ICP; bidirectional sampled residual",
        "metric_status": "unverified",
        "sample_count": len(source),
        "best": refined[0],
        "alternatives": refined[1:4],
    }
