...
from .storage import DATA, read_scene, list_scenes, propose, decide, scene_path, media_path, identifier, event
...
@app.get("/api/health")
def health():
    return {"ok":True,"services":{"wandb_configured":bool(os.environ.get("WANDB_API_KEY")),
                                   "fal_configured":bool(os.environ.get("FAL_KEY"))}}


@app.get("/api/scenes")
def scenes():
    return [{"id":s.id,"title":s.title,"description":s.description,"source_kind":s.source_kind} for s in list_scenes()]


@app.get("/api/scenes/{scene_id}")
def scene(scene_id: str):
    try:return read_scene(scene_id)
    except FileNotFoundError:raise HTTPException(404,"Scene not found")
...
def media(path: str):
    ...
    if p.suffix.lower() not in {".png",".jpg",".jpeg",".mp4",".ply",".spz",".rad",".glb",".splat"}:
...
def decision(scene_id: str, revision_id: str, value: Decision):
    revision=decide(scene_id,revision_id,value.accept,{"source":"human","reason":value.reason})
...
def screenshot(scene_id: str, request: CaptureRequest):
    ...
    if request.camera not in scene.cameras:raise HTTPException(422,"Unknown camera")
    result=capture(scene_id,request.revision or scene.current_revision,request.camera)
...
async def loop(scene_id: str, request: LoopRequest):
    read_scene(scene_id)
    if any(j["scene_id"]==scene_id and j["status"]=="running" for j in JOBS.values()):
    ...
    async def work():
        try:
            def run():
                ...
                initialize_tracing()
                return repair_loop(scene_id,request.max_passes,job)
...
def job(job_id: str):
    if job_id not in JOBS:raise HTTPException(404,"Job not found in this server session")
    return JOBS[job_id]
